    ...
    return (
        <button 
            ...
            className={`group relative text-left flex flex-col rounded-[2.5rem] border overflow-hidden transition-all duration-700 hover:-translate-y-2 animate-fade-in-up
                ${isDark ? 'bg-white/[0.02] border-white/10 hover:bg-white/[0.04]' : 'bg-white border-stone-200 shadow-xl'}
            `}
            ...
            <div className="p-6 bg-black/20 flex-grow border-t border-white/5">
                <div className="flex gap-3">
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed italic font-serif ${isDark ? 'text-slate-400' : 'text-stone-600'} line-clamp-3`}>
                            "{res.reason}"
                        </p>
                    </div>
                </div>
...
const SearchPage: React.FC<SearchPageProps> = ({ moments, onNavigate, onSelectMoment, language }) => {
    const { theme } = useTheme();
    const isDark = theme === 'dark';
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-[#050811]' : 'bg-[#FDFBF7]'} transition-colors duration-500 overflow-x-hidden`}>
            ...
            <main className="container mx-auto px-4 md:px-6 pt-24 pb-40 max-w-7xl relative z-10">
                    ...
                    <div className="relative group">
                        ...
                        `}>
                            ...
                            <input 
                                ...
                                placeholder="Speak to æterny... Search by vibe, meaning, or memory."
                                className={`flex-grow bg-transparent border-none py-6 text-xl md:text-2xl focus:ring-0 font-medium ${isDark ? 'text-white' : 'text-stone-900'} placeholder:text-slate-700`}
                            />
                ...
                {!results.length && !isSearching && moments.length > 0 && (
                    <div className="mt-40 text-center opacity-20 group">
                        ...
                        <p className="text-xs text-slate-600 mt-2">æterny is currently tracking {moments.length} artifacts for your resonance.</p>
                    </div>
                )}
            </main>
