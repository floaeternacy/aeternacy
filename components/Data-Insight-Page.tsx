...
const DataInsightPage: React.FC<DataInsightPageProps> = ({ moments }) => {
    ...
    // Explicitly typed standardMoments as Moment[] to ensure reliable inference
    ...
    // Explicitly typed insights variable to ensure stable property access in JSX and fix potential "unknown" errors
    const insights: {
        ...
        const topPeople = Object.entries(peopleCounts).sort(([,a],[,b]) => b-a).slice(0, 5) as [string, number][];
        const topLocations = Object.entries(locationCounts).sort(([,a],[,b]) => b-a).slice(0, 5) as [string, number][];
        const topActivities = Object.entries(activityCounts).sort(([,a],[,b]) => b-a).slice(0, 3) as [string, number][];
        ...
        <div className="container mx-auto px-6 pt-28 pb-12 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                <div className="bg-gray-800/50 p-6 rounded-2xl ring-1 ring-white/10">
                    ...
                    <ul className="space-y-3">{(insights.topPeople as [string, number][]).map(([name, count]) => <li key={name} className="flex justify-between text-sm"><span className="text-slate-300">{name}</span><span className="font-semibold text-white">{count} moments</span></li>)}</ul>
                </div>
                <div className="bg-gray-800/50 p-6 rounded-2xl ring-1 ring-white/10">
                    ...
                    <ul className="space-y-3">{(insights.topLocations as [string, number][]).map(([loc, count]) => <li key={loc} className="flex justify-between text-sm"><span className="text-slate-300">{loc}</span><span className="font-semibold text-white">{count} times</span></li>)}</ul>
                </div>
                <div className="bg-gray-800/50 p-6 rounded-2xl ring-1 ring-white/10">
                    ...
                    <ul className="space-y-3">{(insights.topActivities as [string, number][]).map(([act, count]) => <li key={act} className="flex justify-between text-sm"><span className="text-slate-300">{act}</span><span className="font-semibold text-white">{count} times</span></li>)}</ul>
                </div>
            </div>
