...
import { Users, LayoutGrid, UserPlus, ArrowRight, GitMerge, Heart, MapPin } from 'lucide-react';
import { ASSETS } from '../data/assets';
import ChevronDoubleDownIcon from './icons/ChevronDoubleDownIcon';

// New static backgrounds for the family page using ASSETS
const staticFamilyBackgrounds = [
    { url: ASSETS.FAMILY.HERO_SLIDES[0], title: 'Shared Laughter', description: 'Moments of joy that become the foundation of your family\'s story.', location: 'Home', people: ['Family'] },
    { url: ASSETS.FAMILY.HERO_SLIDES[1], title: 'Generations Connected', description: 'The timeless bond that links the past, present, and future.', location: 'Reunion', people: ['Grandparents', 'Kids'] },
    { url: ASSETS.FAMILY.HERO_SLIDES[2], title: 'Holiday Warmth', description: 'The cozy comfort of tradition and togetherness.', location: 'Cabin', people: ['Everyone'] },
    { url: ASSETS.FAMILY.HERO_SLIDES[3], title: 'Outdoor Adventures', description: 'Exploring the world together, creating memories one step at a time.', location: 'National Park', people: ['Family'] },
    { url: ASSETS.FAMILY.HERO_SLIDES[4], title: 'A Quiet Moment', description: 'In the simple, shared silence, the deepest connections are felt.', location: 'Living Room', people: ['Parents'] }
];
...
const FamilyHomePage: React.FC<FamilyHomePageProps> = ({ onNavigate, userTier, moments, onSelectMoment }) => {
  ...
  useEffect(() => {
    setBackgrounds(staticFamilyBackgrounds);
  }, []);
