 ...
 * conversational text or markdown formatting.
 */
function safeParseJson<T>(text: string | undefined): T {
    if (!text) return {} as T;
    
    // Attempt to find a JSON block using regex
    ...
    try {
        ...
        // Fallback for failed extraction: try to strip markdown backticks
        try {
            const stripped = cleanedText.replace(/```json|```/g, "").trim();
...
export async function generateVideo(prompt: string, imagePayload: { data: string; mimeType: string }, aspectRatio: '16:9' | '9:16' = '16:9'): Promise<string> {
    ...
    try {
        ...
        if (!response.ok) {
            // Read as text first to avoid stream controller errors during JSON failure
            const errorText = await response.text();
            let errorMessage = "PERMISSION_DENIED";
            try {
        ...
        const msg = (e.message || (typeof e === 'string' ? e : JSON.stringify(e))).toLowerCase();
...
export async function getRelatedMoments(moment: Moment, allMoments: Moment[]): Promise<SearchResult[]> {
    ...
    return safeParseJson<SearchResult[]>(response.text);
}
...
export async function generateAeternyStory(
    ...
    const STYLE_PROMPTS: Record<StoryStyle, string> = {
        ...
        journal: "Write as a detailed documentary or journal entry. Precise, factual, yet deeply personal.",
    ...
    return safeParseJson<{ title: string; story: string; tags: any }>(response.text);
}

export async function rewriteStoryWithStyle(
    currentStory: string,
    mediaItems: { data: string; mimeType: string }[],
    style: StoryStyle
    ...
    const STYLE_PROMPTS: Record<StoryStyle, string> = {
        nostalgic: "Warm, reflective, and sentimental. Looking back with fondness.",
        poetic: "Vivid imagery, artistic metaphors, and lyrical flow. Impactful and deep.",
        journal: "Documentary style. Precise, factual, and chronological. A clear record of life.",
        lighthearted: "Fun, casual, and cheerful. Focus on the smiles and the light moments."
    };

    const prompt = `
        You are æterny, the master biographer. 
        I need you to REWRITE the following life story into a NEW PERSONA.
        ...
        3. Use the provided images for additional sensory context if needed.
        
        Return JSON: {"title": "A new evocative title", "story": "The rewritten narrative"}
    `;
    ...
    return safeParseJson<{ title: string; story: string }>(response.text);
}
...
export async function mergeJourneysIntoEpoch(
    ...
    return safeParseJson<{ title: string; description: string; theme: string }>(response.text);
}
